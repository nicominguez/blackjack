...
def main():
    ...
    # Run simulations
    bh_rand = run_simulation(
        p=RandomStrategyPlayer(), num_hands=num_hands, bet_amount=bet_amount
    )[5]
    bh_basic = run_simulation(
        p=BasicStrategyPlayer(), num_hands=num_hands, bet_amount=bet_amount
    )[5]
    bh_chart1 = run_simulation(
        p=ChartPlayer1(), num_hands=num_hands, bet_amount=bet_amount
    )[5]
    bh_chart2 = run_simulation(
        p=ChartPlayer2(), num_hands=num_hands, bet_amount=bet_amount
    )[5]
